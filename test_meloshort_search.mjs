...
async function testMeloShortSearch() {
    ...
    try {
        ...
        data.forEach((item, i) => {
            console.log(`[${i+1}] ID: ${item.id || item.intId} | Title: ${item.title || item.name || item.book_name} | Platform: ${item.platform || item.provider || 'none'}`);
        });
    } catch (e) {
